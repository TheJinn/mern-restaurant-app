...
export default function Menu(){
  ...
  return (<div className="flex min-h-screen"><NavBar /><main className="flex-1"><div className="max-w-[1100px] mx-auto p-4 space-y-4">
    ...
    <div className="grid md:grid-cols-9 gap-3 items-end">
      ...
      <div><div className="text-sm">Image URL:</div><input className="border rounded px-2 py-1 w-full" value={form.productImage} onChange={e=>setForm(f=>({...f,productImage:e.target.value}))}/></div>
      <div><div className="text-sm">Upload Image:</div><input type="file" accept="image/*" className="block text-sm" onChange={(e)=>{ const file = e.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => setForm(f=>({...f, productImage: reader.result })); reader.readAsDataURL(file); }}/></div>
      ...
      <button className="bg-black text-white px-3 py-2 rounded-full col-span-full md:col-span-1" onClick={createItem}>Create</button>
    </div>
